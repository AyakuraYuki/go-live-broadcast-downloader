...
func New(factory Factory, opts *Options) (*Pool, error) {
	...
	if opts.Init > opts.MaxActive {
		opts.Init = opts.MaxActive
	}
	p := &Pool{
		clients:     make(chan ClientConn, opts.MaxActive),
		factory:     factory,
		idleTimeout: opts.IdleTimeout,
	}
	if opts.MaxLifeDuration > 0 {
		p.maxLifeDuration = opts.MaxLifeDuration
	}
	for i := 0; i < opts.Init; i++ {
		c, err := factory()
		if err != nil {
			return nil, err
		}
	...
	for i := 0; i < opts.MaxActive-opts.Init; i++ {
		p.clients <- ClientConn{
			pool: p,
		}
	}
...
func (c *ClientConn) Close() error {
	...
	if maxDuration > 0 && c.timeInitiated.Add(getRandDuration(maxDuration)).Before(time.Now()) {
		c.Unhealthy()
	}
...
func getRandDuration(maxDuration time.Duration) time.Duration {
	return maxDuration + time.Duration(30+rand.Int63n(120))*time.Second
}
